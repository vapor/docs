...
기본 값으로 구성된 템플릿으로서 `.env` 파일을 프로젝트에 포함하는 것은 일반적인 방식입니다. 특정 환경 파일은 .`.gitignore`에서 다음 패턴으로 업로드되지 않도록 합니다.
...
arguments 배열에는 실행 가능한 이름을 나타내는 인자를 최소한 하나는 포함해야 합니다. 명령 줄에 인자를 제공하는 것 같은 시뮬레이션을 하기 위해서 추가적인 인자들을 제공할 수 있습니다. 특히 테스트를 위해서 사용할 때 유용합니다.
