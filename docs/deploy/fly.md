...
This command will display an interactive prompt that will let you configure your Vapor project. We recommend selecting Fluent with a Postgres database type if you need a database; Fly makes it easy to create a managed Postgres database to connect your apps to (see the [dedicated section](#configuring-postgres) below).
