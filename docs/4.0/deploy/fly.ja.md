...
まず、Vaporツールボックスがインストールされていることを確認してください（[macOS](../install/macos.md#toolbox-のインストール-install-toolbox)または[Linux](../install/linux.md#ツールボックスのインストール-install-toolbox)のインストール手順を参照）。
