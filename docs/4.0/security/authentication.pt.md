...
Exigir autenticação não é feito pelo middleware do authenticator para permitir composição de authenticators. Leia mais sobre [composição](#composição) abaixo.
...
!!! note "Nota"
    O cookie de sessão não é definido como `secure` e `httpOnly` por padrão. Consulte a [API de Sessão](../advanced/sessions.md#configuração) do Vapor para mais informações sobre como configurar cookies.
