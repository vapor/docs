...
`prepare` 方法是对提供的 `Database` 进行更改的地方。这些可能是对数据库模式的更改，如添加或删除表或集合、字段或约束。他们还可以修改数据库内容，比如创建新的模型实例、更新字段值或进行清理。
...
请查看[模式构建器](schema.md) 和 [查询构建器](query.md) 指南，以了解更多迁移相关的信息。
