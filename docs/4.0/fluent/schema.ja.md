...
最初のパラメータはフィールドの名前です。これは、関連するモデルプロパティで使用されるキーと一致する必要があります。2番目のパラメータはフィールドの[データ型](#データ型-data-type)です。最後に、0個以上の[制約](#フィールド制約-field-constraint)を追加できます。
...
スキーマの作成または更新時に制約を追加できます。[フィールド制約](#フィールド制約-field-constraint)とは異なり、トップレベルの制約は複数のフィールドに影響を与えることができます。
...
フィールド制約とは異なり、トップレベルの制約はスキーマ更新で追加できます。また、[名前を付ける](#制約名-constraint-name)こともできます。
