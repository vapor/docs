...
只有当 `RawValue` 为 `String` 时，遵循 `RawRepresentable` 协议的类型才与 `@Enum` 兼容。默认情况下，由 `String` 支持的枚举满足此要求。
...
!!! info "信息"
    我们建议在几乎所有情况下，都使用 DTO 而不是模型来处理你的 API 响应和请求体。更多信息请参阅[数据传输对象](#data-transfer-object)。

### 数据传输对象

模型默认遵循 `Codable` 协议可以让简单用法和原型开发更容易。然而，这会将底层数据库信息暴露给 API。这通常是不可取的，无论是从安全角度——返回诸如用户密码哈希之类的敏感字段是个坏主意——还是从可用性角度来看都是如此。这会使得在不破坏 API 的情况下更改数据库模式、以不同格式接受或返回数据，或者向 API 添加或删除字段变得困难。

在大多数情况下，你应该使用 DTO，即数据传输对象，来代替模型(这也被称为领域传输对象)。DTO 是一个单独的 `Codable` 类型，表示你想要编码或解码的数据结构。这些类型将你的 API 与数据库模式解耦，使你可以在不破坏应用程序公共 API 的情况下修改模型、拥有不同的版本，并让你的 API 对客户端更友好。

接下来的示例中，假设有以下 `User` 模型。
...
另一个常见用例是处理关系，比如父子关系。请参阅[父级文档](relations.zh.md#encoding-and-decoding-of-parents)，了解如何使用 DTO 轻松解码带有 `@Parent` 关系的模型的示例。

即使 DTO 的结构与模型的 `Codable` 协议一致性完全相同，将其作为一个单独的类型也有助于保持大型项目的整洁。如果你需要修改模型的属性，也不必担心破坏应用程序的公共 API。你还可以考虑将 DTO 放在一个单独的包中，该包可以与你的 API 使用者共享，并在你的 Vapor 应用中为其添加 `Content` 一致性。
