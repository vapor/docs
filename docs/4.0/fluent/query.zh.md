...
查询构建器与单个模型类型相关联，可以使用静态 [`query`](model.zh.md#查询query) 方法创建。也可以通过将模型类型传递给数据库对象上的 `query` 方法来创建它们。
...
!!! tip "建议" 
    如果使用`EventLoopFuture`，此方法可以与 [`unwrap(or:)`](../basics/errors.zh.md#中断abort) 组合使用以返回非可选模型或抛出错误。
