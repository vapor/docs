...
フィールド値は、`Codable`に準拠する任意の型にできます。ネストされた構造体や配列を`@Field`に格納することはサポートされていますが、フィルタリング操作は制限されます。代替案については[`@Group`](#グループ-group)を参照してください。
...
|`.delete`|モデルがデータベースから削除されるときに設定される。[論理削除](#論理削除-soft-delete)を参照|
...
!!! info
    ほぼすべてのケースで、APIレスポンスとリクエストボディにはモデルの代わりにDTOを使用することをお勧めします。詳細については[データ転送オブジェクト](#データ転送オブジェクト-data-transfer-object)を参照してください。
...
もう1つの一般的な使用例は、親リレーションや子リレーションなどのリレーションを扱う場合です。`@Parent`リレーションを持つモデルを簡単にデコードするためのDTOの使用例については、[Parentドキュメント](relations.md#parent-のエンコードとデコード-encoding-and-decoding-of-parents)を参照してください。
...
!!! warning
    `.database`ジェネレーター（通常は自動インクリメントの`Int`）を使用する[`@ID(custom:)`](#カスタム識別子-custom-identifier)を使用するモデルは、バッチ作成後に新しく作成された識別子にアクセスできません。識別子にアクセスする必要がある状況では、各モデルで`create`を呼び出してください。
