...
Si no se proporciona un [`RedisSessionsDelegate`](https://api.vapor.codes/redis/redissessionsdelegate) personalizado, se usará uno predeterminado.
...
> Documentación de la API: [`RedisSessionsDelegate`](https://api.vapor.codes/redis/redissessionsdelegate)

Un objeto que cumple con este protocolo puede usarse para cambiar cómo `SessionData` se almacena en Redis.

Solo dos métodos requieren de implementación en un tipo que cumpla con el protocolo: [`redis(_:store:with:)`](https://api.vapor.codes/redis/redissessionsdelegate/redis(_:store:with:)) y [`redis(_:fetchDataFor:)`](https://api.vapor.codes/redis/redissessionsdelegate/redis(_:fetchdatafor:)).
