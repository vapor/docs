...
Als er geen aangepaste [`RedisSessionsDelegate`](https://api.vapor.codes/redis/redissessionsdelegate) is opgegeven, zal er een standaard worden gebruikt.
...
> API Documentatie: [`RedisSessionsDelegate`](https://api.vapor.codes/redis/redissessionsdelegate)

Een object dat voldoet aan dit protocol kan worden gebruikt om te veranderen hoe `SessionData` wordt opgeslagen in Redis.

Slechts twee methoden hoeven te worden geïmplementeerd door een type dat voldoet aan het protocol: [`redis(_:store:with:)`](https://api.vapor.codes/redis/redissessionsdelegate/redis(_:store:with:)) en [`redis(_:fetchDataFor:)`](https://api.vapor.codes/redis/redissessionsdelegate/redis(_:fetchdatafor:)).
