...
|`.url`|有効な URL を含みます。|
|`.custom(_:, validationClosure: (value) -> Bool)`|その場限りのカスタムバリデーション。|

バリデーターはまた、演算子を使用して複雑な検証を組み立てるために組み合わせることができます。`.custom` バリデーターの詳細については、[カスタムバリデーション](#カスタムバリデーション-custom-validators)を参照してください。
...
Validation API の拡張は、カスタムバリデーターを複数の `Content` オブジェクトで使用する予定がある場合に最も適しています。このセクションでは、郵便番号を検証するカスタムバリデーターを作成する手順を説明します。
