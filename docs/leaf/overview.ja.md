...
Leaf は、Swift 風の構文を持つ強力なテンプレート言語です。これを使って、フロントエンドのウェブサイト用に動的な HTML ページを生成したり、API から送信するリッチなメールを生成したりすることができます。
...
- 名前 `coun` : タグを識別するための名前です
- パラメータリスト `(users)` : 0個以上の引数を受け取ることができます
- ボディ：一部のタグには、コロンと閉じタグを使用して任意の body を提供できます

これらの4つの要素は、タグの実装に応じて様々な使い方ができます。 Leaf のビルドインタグがどのように使われるか、いくつかの例をみてみましょう。
